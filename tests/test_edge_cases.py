...
class TestErrorHandling:
    ...
    def test_is_expired_invalid_deadline_type(self, tmp_path):
        ...
        with pytest.raises(TypeError):
            is_expired(test_file, "10")  # 文字列型は無効
            
        with pytest.raises(TypeError):
            is_expired(test_file, [10])  # リスト型は無効
            
        with pytest.raises(TypeError):
            is_expired(test_file, {})  # 辞書型は無効
    ...
    def test_remove_expired_file_exceptions(self, tmp_path):
        ...
        # パーミッションエラーをシミュレート
        with patch("pathlib.Path.unlink", side_effect=PermissionError("権限がありません")):
...
class TestFormatHandling:
    ...
    def test_extract_date_from_filename_edge_cases(self):
        ...
        # KeyErrorが発生するケース
        with patch("re.search", return_value=type("obj", (object,), {
        ...
        mock_match = type("obj", (object,), {
            ...
            "group": lambda self, name: {"year4": "2025", "month": "02", "day": "30"}.get(name, "01")
        })()
        ...
        mock_date_match = type("obj", (object,), {
            "groupdict": lambda self: {"year4": "abcd", "month": "xy", "day": "zz"},
            "group": lambda self, name: {"year4": "abcd", "month": "xy", "day": "zz"}.get(name, "01")
        })()
    ...
    def test_remove_expired_files_file_filter(self, tmp_path):
        ...
        assert (tmp_path / "old5.txt.bak").exists()  # .txt.bakファイルは残る（.txtではない）
        
        # サフィックスがない場合のテスト（行97のカバレッジのため）
        empty_file = tmp_path / ".hidden"  # 隠しファイル（サフィックスなしエッジケース）
        empty_file.touch()
        old_time = datetime.now() - timedelta(days=10)
    ...
    def test_remove_by_filename_date_error_cases(self, tmp_path):
        ...
        with patch("os.access", return_value=True):  # アクセス権チェックをバイパス
            with patch("expired_file_remover.core.is_filename_date_expired", 
...
class TestExceptionHandling:
    ...
    def test_remove_expired_files_by_filename_date_os_error(self, tmp_path):
        ...
        with patch("pathlib.Path.unlink", side_effect=OSError("OSエラー")):
            # エラーが出てもカウントは0で返る
            assert remove_expired_files_by_filename_date(test_dir, "%Y%m%d", datetime(2025, 2, 1)) == 0
    ...
    def test_remove_expired_files_by_filename_date_permission_error(self, tmp_path):
        ...
        with patch("pathlib.Path.unlink", side_effect=PermissionError("権限がありません")):
            with pytest.raises(PermissionError) as e:
                remove_expired_files_by_filename_date(test_dir, "%Y%m%d", datetime(2025, 2, 1))
